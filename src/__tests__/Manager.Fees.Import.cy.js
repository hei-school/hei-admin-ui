...
describe(specTitle('Manager import fees for one students'), () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', `/students?page=1&page_size=10&first_name=${student1Mock.first_name}`, [student1Mock]).as('getStudentsByFirstName')
    cy.intercept('GET', `/students/${student1Mock.id}`, student1Mock).as('getStudent1')
    cy.intercept('GET', `/students/${student1Mock.id}/fees?page=1&page_size=10`, feesMock).as('getStudent1Fees')
