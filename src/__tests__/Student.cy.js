...
describe(specTitle('Student'), () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', `/students/${student1Mock.id}/fees/${feesMock[11 - 1].id}/payments?page=1&page_size=10`, createPaymentMock(feesMock[11 - 1])).as(
      'getPaymentsOfFee1'
    )
    ...
    cy.intercept('GET', `/students/${student1Mock.id}/fees/${feesMock[11 - 1].id}`, feesMock[11 - 1]).as('getFee1')
