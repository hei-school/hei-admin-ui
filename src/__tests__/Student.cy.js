...
describe(specTitle('Student'), () => {
  ...
  it('shows latitude and longitude when they are available', () => {
    cy.get('#location').contains('Location: \n' + student1Mock.location.latitude + '\n' + student1Mock.location.longitude)
  })
  it('shows that it is not available', () => {
    cy.intercept('GET', `/students/${student1Mock.id}`, student1MockWithoutLocation).as('getStudent')
    cy.get('#location').contains('Indisponible')
  })
