...
import {
  ...
  leaveGroupFlow,
  migrateGroupFlow,
} from "./mocks/responses/group-flow-api";
import {
  editedGroup,
  group1Students,
  groups,
  newGroup1,
  student1,
} from "./mocks/responses/groups-api";
...
describe(specTitle("Manager and groups"), () => {
  beforeEach(() => {
    ...
    cy.intercept("GET", `/students?page=1&page_size=25`, studentsMock).as(
      "getStudents"
    );
    cy.intercept("GET", `/groups?page=1&page_size=10&ref=${group1.ref}`, [
      group1,
    ]).as("getFilteredGroup");
    cy.intercept("GET", `/groups/${group1.id}`, group1).as("getGroup1");
    cy.intercept(
      "GET",
      `/groups/${group1.id}/students?page=1&page_size=10`,
      group1Students
    ).as("getGroupStudents");
  ...
  it("can edit a group", () => {
    ...
    cy.intercept("GET", `/groups?page=1&page_size=10`, newGroupList).as(
      "getGroups"
    );
  ...
  it("can remove a specific student from a group students list", () => {
    cy.intercept(
      "POST",
      `/students/${student.id}/group_flows`,
      leaveGroupFlow
    ).as("removeStudent");
  ...
  it("can add a student to a group", () => {
    cy.intercept(
      "POST",
      `/students/${student1.id}/group_flows`,
      addGroupFlow
    ).as("removeStudent");
