...
const transformUser = (user) => {
  ...
  let condition =
    !regex.test(user.entrance_datetime) && !regex2.test(user.entrance_datetime);
  user.entrance_datetime = condition
    ? user.entrance_datetime.concat("T10:00:00.000Z")
    : user.entrance_datetime;
  return user;
};
