...
export type EventCardPropsType = {
  event: Required<Event> & {title?: string, planner: Pick<Event, "planner"> & {pic?: string}}
}
...
export function EventCard({event}: EventCardPropsType) {
  ...
  return (
    <Card sx={{width: "30%", overflow: "visible","& *": {bgcolor: "white !important"}, bgcolor: "white !important", p: 2, borderRadius: "8px"}}>
      <Box sx={{display: "flex", overflowY: "visible", alignItems:"end", justifyContent: "space-between", width: "100%"}}>
        <Box sx={{display: "flex", alignItems:"center", gap: 2}}>
          <EventIcon sx={{fontSize: "2rem"}} />
          <Typography sx={{fontWeight: "bold", color: "#454443", fontSize: "14px"}}>
            Presence L1
          </Typography>
        </Box>
      ...
      <CardContent>
        <Typography sx={{fontWeight: "bold", color: "#454443", fontSize: "16px"}}>
          Présence  L1
        </Typography>
        {event.description}
      </CardContent>
