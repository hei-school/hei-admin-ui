...
const getChipColor = (scope : string) => {
    switch (scope) {
        ...
        default:
            return "Portée indéfinie"
    }
}

const ScopeField: FC<ScopeFieldProps> = ({scope}: ScopeFieldProps) => {
    return <Chip label={ANNOUNCEMENT_SCOPE[scope]} sx={{ backgroundColor: getChipColor(scope), color: PALETTE_COLORS.white}}/>
}
...
export const AnnouncementList = () => {
    return (
        ...
        >
            ...
            <FunctionField label="Portée de l'annonce" render={(record: Announcement) => <ScopeField scope={record?.scope!} />}/>
