...
const WizardToolbar = () => {
  ...
  return (
    <div style={{display: 'flex', justifyContent: 'space-around'}}>
      {hasPreviousStep && <Button onClick={() => goToPreviousStep()} startIcon={<ArrowBackIos/>} style={{display: 'flex', justifySelf: 'start'}} variant='contained' size='small'>Retour</Button>}
      {hasNextStep && (<Button onClick={() => goToNextStep()} endIcon={<ArrowForwardIos/>} style={{display: 'flex', justifySelf: 'end'}} variant='contained' size='small'>Suivant</Button>)}
    </div>
  );
};
      ...
      <DialogContent>
        <WizardForm toolbar={<WizardToolbar/>} >
          <WizardFormStep label='Fichier' >
            ...
            <Typography variant='body2'>Vous pouvez choisir le nom de votre template.
              Par défaut, ce sera <strong>template.xlsx</strong></Typography>
          ...
          <WizardFormStep label='En-têtes'>
            <Grid container padding={3} rowSpacing={2}>
              <Grid item >
                <Typography variant='body2'>Vous pouvez aussi personnaliser les en-têtes.
                  Les en-têtes déjà cochées sont obligatoires.
                  <br/>Il ne vous restera plus qu'à appuyez sur le bouton <strong>TÉLÉCHARGER</strong> et modifier votre template.</Typography>
              </Grid>
              <Grid item>
                <FormGroup>
                  {
                    defaultHeaders.map((head) => <FormControlLabel key={head.id} control={<Checkbox size='small' disabled={head.disabled} checked {...register('importHeaders', {})}/>} value={head.value} label={<Typography variant="body2">{head.label}</Typography>} />)
                  }
                  {
                    optionalHeaders.map((head) => <FormControlLabel key={head.id} control={<Checkbox size='small' {...register('importHeaders', {})}/>} value={head.value} label={<Typography variant="body2">{head.label}</Typography>}  />)
                  }
                </FormGroup>
                <Button onClick={handleSubmit(downloadFile)} startIcon={<Download />} size='medium' variant='outlined'>Télécharger</Button>
              </Grid>
            </Grid>
          </WizardFormStep>
          <WizardFormStep label='Importation'>
            <Grid container padding={3} rowSpacing={5}>
              <Grid item>
                <Typography variant='body2'>Ça y est?
                  Après avoir vérifié votre fichier, appuyez sur le bouton <strong>IMPORTER</strong> et voilà ! 😁</Typography>
              </Grid>
              <Grid item>
