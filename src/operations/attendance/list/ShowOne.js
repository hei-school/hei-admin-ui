...
function Title({content, label, link}){
  const style = {color:'#4a4b4d',my:.4,fontSize:'14px'}
  const getContent = !link 
    ?<span style={{color:'#282829'}}>{content}</span>
    :<Link to={link}>{content}</Link>

  return <Typography sx={style}>{label}: {getContent}</Typography>
}
...
function ShowOne({ showOne, setShowOne }){
  ...
  const teacher = (record && record.course_session.id) ?
          ...
          <DialogContent sx={{minWidth:'300px'}}>
            ...
            <Title label='Lieu' content={record.place}/>
            {
              record.course_session.id && <>
                ...
                {data.map((el, index)=><Title key={index} {...el}/>)}
              </>
            }
          </DialogContent>
        </>
