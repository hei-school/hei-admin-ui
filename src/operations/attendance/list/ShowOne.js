...
function Title({ content, label, link }) {
  ...
  const getContent = !link ? <span style={{ color: '#282829' }}>{content}</span> : <Link to={link}>{content}</Link>
...
function ShowOne({ showOne, setShowOne }) {
  ...
  const student = record ? `${record.student.first_name} ${record.student.ref}` : ''
  const teacher =
    record && record.course_session.id
      ? `${record.course_session.awareded_course.main_teacher.first_name} ` + record.course_session.awareded_course.main_teacher.ref
      : ''

  const data = !(record && record.course_session.id)
    ? []
    : [
        { label: 'Group', content: record.course_session.awareded_course.group.ref },
        { label: 'Cours', content: record.course_session.awareded_course.course.code },
        { label: 'Enseignant', link: `/teachers/${record.course_session.awareded_course.main_teacher.id}/show`, content: teacher },
