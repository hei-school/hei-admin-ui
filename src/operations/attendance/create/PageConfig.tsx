...
export const QrPageConfig: FC<PageConfigProps> = ({open, toggle}) => {
  ...
  const handlerNewConfig = (event: any) => {
  ...
  const changeQrConfig = (configValue: any) => {
  ...
  return (
    <>
      ...
      <Dialog onClose={toggleShowConfig} sx={{width: "100%"}} open={open}>
        ...
        >
          <Typography
            sx={{color: "#232423", fontSize: ".9em", fontWeight: "bold"}}
          >
        ...
        <DialogContent sx={{minWidth: "300px", px: 2}}>
          {inputs.map((el) => (
            <ConfigInput key={el.id} handler={handlerNewConfig} {...el} />
          ))}
          <FormGroup>
            ...
            >
              {AvailablePlace.map((place, id) => (
                <MenuItem key={id} value={place.value} sx={{fontSize: "15px"}}>
                  {place.label}
                </MenuItem>
              ))}
            </Select>
        ...
        <DialogActions>
          <Button
            onClick={() => changeQrConfig(defaultConfig)}
            size="small"
            variant="outlined"
            color="info"
          >
          ...
          <Button
            onClick={() => changeQrConfig(newConfig)}
            size="small"
            variant="outlined"
            color="primary"
          >
