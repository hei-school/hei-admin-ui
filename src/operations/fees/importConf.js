...
export const optionalFeesHeaders = [{ id: 4, label: 'Commentaire (comment)', value: 'comment', disabled: false }]
...
export const valideFeesData = data => {
  ...
  if (response.isValid) {
    ...
    if (data.some(el => isNaN(Number(el.due_datetime)))) response.message = "Certain(s) date limite(s) n'est (ne sont) pas valide(s)."
    else if (data.some(el => isNaN(Number(el.total_amount)))) response.message = 'Tous les montants totaux doivent être des nombres'
