...
export function PredefinedFeeTypeRadioButton({setFeesConf, ...props}) {
  return (
    <SelectInput
      ...
      choices={Object.keys(predefinedFeeTypes).map((id) => ({
        id: id,
        name: predefinedFeeTypes[id][0].name,
      }))}
...
export function PredefinedFirstDueDateRadioButton({createFeesConf, ...props}) {
  ...
  const showFirstDateInput = ({target}) =>
    setIsLastDay(target.value === "date3");
  const updateFirstDate = ({target}) =>
    setFirstDate({...firstDate, [target.name]: target.value});
  ...
  return (
    <Box sx={{display: "flex", alignItems: "start", gap: 1}}>
      ...
      />
      {isLastDay && (
        <DateValueInput dateValue={firstDate} onChange={updateFirstDate} />
      )}
    </Box>
  );
}
