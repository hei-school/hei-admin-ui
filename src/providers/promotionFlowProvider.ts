...
const promotionFlowsProvider: HaDataProviderType = {
  ...
  async saveOrUpdate(payload: UpdatePromotionSGroup[], meta: { promotionId: string }) {
    if (payload.length <= 0) {
      throw new Error("Cannot update empty list of promotions");
    }
    return promotionApi().updatePromotionGroups(meta.promotionId, payload[0]).then(response => [response.data]);
  },
